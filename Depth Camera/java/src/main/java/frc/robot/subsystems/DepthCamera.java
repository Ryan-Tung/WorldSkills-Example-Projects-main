    ...
    import com.orbbec.obsensor.*;  //file for camera
...
public class DepthCamera extends SubsystemBase
{
    ...
    private boolean grabFrameInit = false;
    ...
    // /**
    //  * Decode the color frame and publish it as an MJPEG stream for the dashboard.
    //  * 
    //  * @param frame ColorFrame from the pipeline (expected MJPG format)
    //  * /
    ...
    private void streamColorFrame(ColorFrame frame)
    {
        ...
        if (!colorStreamInit)
        {
            colorStream = CameraServer.getInstance().putVideo("Orbbec RGB", frame.getWidth(), frame.getHeight());
        ...
        Mat encoded = new Mat(1, frameData.length, CvType.CV_8UC1); // 8-bit unsigned integer, single-channel matrix
    ...
    // {
    //     // 1. Create the unique CvSource window once we know the real frame size
    //     if (!grabFrameInit)
    //     {
    //         // Named "Screenshot" to match your class object declaration
    //         colorStream = CameraServer.getInstance().putVideo("Screenshot", frame.getWidth(), frame.getHeight());
    //         grabFrameInit = true;
    //     }

    //     // 2. Read the Shuffleboard toggle switch trigger
    //     boolean shouldSave = SmartDashboard.getBoolean("Capture Frame", false);

    //     // 3. Only run the decoding and saving process if the trigger is flipped true
    //     if (shouldSave)
    //     {
    //         // Pull the raw JPEG bytes out of the frame
    //         byte[] frameData = new byte[frame.getDataSize()];
    ...
    //         if (bgr != null && !bgr.empty())
    //         {
    //             // Absolute path to your Raspberry Pi desktop
    //             String desktopPath = "/home/pi/Desktop/2345.png";
                
    //             // Native OpenCV write command to save the file
    //             Imgcodecs.imwrite(desktopPath, bgr);
    //             System.out.println("Frame successfully saved to Desktop: 2345.png");

    //             // Immediately force the dashboard switch back to false so it executes only once
    //             SmartDashboard.putBoolean("Capture Frame", false);
    //         }

    //         if (bgr != null)
    //         {
    //             bgr.release(); // Free native memory immediately after saving
    //         }
    //     }
